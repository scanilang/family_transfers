...
black_transfer_out = lm(log(sum_transfer_out) ~ log_nonasset_income  + midpoint_age + midpoint_age2 + avg_FU_size + Marital_Status + received_transfer_past + provided_transfer_past + Birth_Cohort + Year_bins,
                        data = psid_model_data %>% filter(Race_Head == "Black") %>%  filter(Provided_Support_Indicator == 1,!is.na(provided_transfer_past), sum_asset_income_threshold ==0))

black_transfer_in = lm(log(sum_transfer_in) ~ log_nonasset_income  + midpoint_age + midpoint_age2 + avg_FU_size + Marital_Status + received_transfer_past + provided_transfer_past + Birth_Cohort + Year_bins,
                       data = psid_model_data %>% filter(Race_Head == "Black") %>%  filter(Received_Support_Indicator == 1,!is.na(provided_transfer_past)), sum_asset_income_threshold ==0) 

white_transfer_out = lm(log(sum_transfer_out) ~ log_nonasset_income  + midpoint_age +midpoint_age2 +  avg_FU_size + Marital_Status + received_transfer_past + provided_transfer_past + Birth_Cohort + Year_bins,
                        data = psid_model_data %>% filter(Race_Head == "White") %>%  filter(Provided_Support_Indicator == 1,!is.na(provided_transfer_past)), sum_asset_income_threshold ==0)

white_transfer_in = lm(log(sum_transfer_in) ~ log_nonasset_income  + midpoint_age + midpoint_age2 + avg_FU_size + Marital_Status + received_transfer_past + provided_transfer_past + Birth_Cohort + Year_bins,
                       data = psid_model_data %>% filter(Race_Head == "White") %>%  filter(Received_Support_Indicator == 1,!is.na(provided_transfer_past)), sum_asset_income_threshold ==0)
